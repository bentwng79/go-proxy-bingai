...
<template>
  <NModal class="w-11/12 lg:w-[900px]" v-model:show="isShowChatServiceSelectModal" preset="card" title="伺服器設置">
    <NTable striped>
      <tbody>
        <tr v-for="(config, index) in sydneyConfigs" :key="index">
          <td>
            ...
            <NInput class="lg:hidden" v-if="config.isCus" v-model:value="config.baseUrl" placeholder="自訂伺服器鏈接" @change="handleChangeSydneyConfig(config)"></NInput>
          </td>
          <td class="hidden lg:table-cell">
            <NInput v-if="config.isCus" v-model:value="config.baseUrl" placeholder="自訂伺服器鏈接" @change="handleChangeSydneyConfig(config)"></NInput>
